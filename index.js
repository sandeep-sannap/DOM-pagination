...
function displayItems(data, wrapper, row_per_page, page) {
  wrapper.innerHTML = "";
  page--;
  ...
  for (let i = 0; i < paginatedItems.length; i++) {
    ...
    item_container.append(name, email);

    wrapper.appendChild(item_container);
  }
}

function paginationSetup(items, wrapper, row_per_page) {
  wrapper.innerHTML = "";
...
function PaginationButton(page, items) {
  ...
  button.addEventListener("click", function () {
    currentPage = page;
    displayItems(items, list_element, row, currentPage);

    let current_btn = document.querySelector(".btn.active");
...
displayItems(data, list_element, row, currentPage);

paginationSetup(data, pagination_element, row);
